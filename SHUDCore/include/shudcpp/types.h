...
namespace SHUD {
    ...
    struct Vec<uint8, 3> {
        ...
        constexpr Vec(uint32 col) : 
            r{ static_cast<uint8>((col & 0x00FF0000) >> 16) },
            g{ static_cast<uint8>((col & 0x0000FF00) >> 8) },
            b{ static_cast<uint8>(col & 0x000000FF) } 
        {};
    ...
    struct FormattedText {
        FormattedText(const char* c) : mText(c) {}
        FormattedText(const std::string& c) : mText(c) {}
        FormattedText(std::string&& c) : mText(c) {}
        ...
        FormattedText(const std::string& fmc, Args ... args) {
            ...
            const_cast<std::string&>(mText) = std::string(buf, buf + size - 1); // We don't want the '\0' inside
            delete[] buf;
        }
