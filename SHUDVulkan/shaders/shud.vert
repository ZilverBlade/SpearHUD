...
void main() {
	bool msdfText = bool(push.mFlags & GHUD_DRAW_DATA_FLAG_IS_MSDF_TEXT);
	...
	if (msdfText) {
		...
		bool italic = bool(msdfData.mFlags & FONT_FORMATTING_FLAG_ITALIC);
		if (italic) {
			float scy = vtx.y * 0.5 + 0.5;
			vtx.x += scy * FONT_ITALICS_FACTOR;
		}
